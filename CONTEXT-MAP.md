...
- **Desktop AI Creation → Object Storage**: Electron Main 只凭 Go 签发的短时、单对象、禁止覆盖的 Reference Material Upload URL 从用户选择的本地文件流式 PUT 到当前实例唯一的阿里云 OSS bucket；Renderer 不接触路径、文件字节或签名 URL，Go finalize 前不存在 Reference Material，下载仍经 Go
...
- **Server → Object Storage**: 每个 Deployment Instance 最多一条 Object Storage Connection，V1 provider 固定为阿里云 OSS；Go 独占 AK/SK、配置验证、对象读取/删除、上传 finalize 与 Provider Transfer Object 授权
...
完整职责和非目标见 [ADR-0014](./docs/adr/0014-go-sole-trusted-data-plane.md)、[ADR-0013](./docs/adr/0013-onprem-single-tenant-delivery.md) 与 [ADR-0016](./docs/adr/0016-ai-creation-v1-trusted-seams.md)。PostgreSQL、AI 供应商与对象存储后端是外部基础设施，不是新的 bounded context。
