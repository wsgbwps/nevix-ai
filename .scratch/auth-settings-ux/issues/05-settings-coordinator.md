...
- [x] 从普通业务视图新进入 Settings 时默认为 Profile，且任一时刻只有一个 Settings Section 挂载。
- [x] Profile 与 Language 选择只 replace 当前 Settings entry，Router Back 不回放 Section 点击历史。
- [x] 两个都指向 `/`、但具有不同 history key 和 source descriptor 的 memory-history entry 都能返回它们各自的原始 entry，证明没有硬编码跳转 Home。
- [x] source 仍存在且可进入时，Router Back 和“返回应用”经同一 intent 返回原 entry；source 失效时 replace 到 Home。
- [x] Profile 每次 mount 重读权威值，使用 last-successful-write-wins；保存失败保留草稿并显示可重试错误。
- [x] dirty Profile 在 Section switch、back、leave 和 ordinary close 前只显示“继续编辑”与“丢弃更改”；确认框不触发保存。
- [x] Profile saving 期间禁止所有应用内导航 intent；ordinary close 等待结果，成功才继续，失败则取消关闭、保留 Settings 与草稿。
- [x] Language 继续即时保存，永不进入 dirty confirmation。
- [x] 离开 Section 卸载其 UI/request state；重进时重新读取；重启或崩溃恢复不恢复 Settings、Section、source 或草稿。
- [x] `Settings Information Architecture Desktop E2E`、Desktop lint/typecheck/build 与 packaged localization 通过。
- [x] 属于安全或 public interface 的产品代码前记录短实施计划；最后代码修改后绑定 final-state evidence 并关闭 finding ledger。
