...
- [x] 启动时最高优先级系统语言既非中文也非英文时，Interface Language 回退为 `zh-CN`。
- [x] 本次启动解析出的系统语言保持稳定，不监听运行期间的操作系统语言变化。
- [x] 主进程拥有解析后的 Interface Language，渲染界面与至少一个主进程拥有的 Localized Surface 显示一致语言。
- [x] 使用 `i18next` 初始化主进程本地化实例，使用 `i18next` 与 `react-i18next` 初始化独立的渲染进程实例；两个进程不共享内存实例。
- [x] 应用 Provider 层向 React 界面提供翻译能力，用户不会看到空白或内部翻译 key。
- [x] 唯一 Supported Language 注册表只声明 `zh-CN` 和 `en`，正式可用语言来源于该注册表，而不是自动扫描全部语言资源。
- [x] 各资源所有者使用独立 namespace；Feature 资源通过其公共边界提供，不直接导入其他 Feature 的内部资源。
- [x] CI 逐 namespace 验证所有 Supported Language 的必需资源存在、必需 key 完整且值非空。
- [x] 未注册候选语言允许资源不完整，且不会自动成为 Supported Language。
- [x] 正式资源意外缺词时，生产配置回退到简体中文；开发和测试配置报告可定位的缺失翻译错误。
