 ...
 * snapshot plus the controller, whose method identities are stable for the
 * connected runtime's lifetime. `getSnapshot` on the controller is the
 * same-tick-fresh read for orchestration callbacks; the binding's snapshot is
 * the committed one. Undefined controller means no runtime. */
 ...
 * instance and the previous lifecycle's in-flight reads retire in the effect
 * cleanup. The constructor is passive, so memoized creation has no render
 * side effects. */
