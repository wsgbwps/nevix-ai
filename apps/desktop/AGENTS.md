...
- Put IPC declarations and named request/response types in `shared/ipc/<domain>/types.ts`. Keep `preload/` generic: expose typed bridge primitives, but do not add per-Domain code or a central Domain registry. The sole security exception is the Creation native-file upload bridge fixed by ADR-0014/0016: it accepts a Renderer `File`, resolves its path with `webUtils.getPathForFile`, sends the path only to Main over its dedicated typed Channel, and never exposes either that raw-path Channel or the path through generic `invoke`
