...
- Keep Domain-specific renderer code under `renderer/src/features/<domain>/` and only the public `index.ts` file at the Feature root. Group implementation by cohesive responsibility; `components/` contains UI components, while distinct non-UI concerns belong in clearly named sibling directories
