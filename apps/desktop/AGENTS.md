...
- Put a page owned by a business Domain in `renderer/src/features/<domain>/`, exported through that Feature's public `index.ts`, and assembled by a thin route; per [ADR-0004](docs/adr/0004-renderer-routing-topology.md), authenticated views render in the App Shell content area except the Organization pre-shell onboarding route, which is the sole documented full-screen exception before an Organization context exists
