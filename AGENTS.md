...
- Delivery is PR-based: push the branch, open a PR against `main`, wait for the path-aware `CI gate`, then squash-merge — see [docs/agents/delivery.md](docs/agents/delivery.md). Exception: changes confined to `.pi/`, `.codex/`, `.agents/`, `.omp/`, `.scratch/`, `docs/`, root `*.md`, or nested `AGENTS.md` files may be committed and pushed directly to `main` without CI
