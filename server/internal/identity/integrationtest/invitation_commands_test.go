...
func TestResendInvitationDoesNotRevalidateHistoricalCode(t *testing.T) {
	...
	status, body, headers := invitationCommandFromIP(t, handler, http.MethodPost, acceptPath, inviteeToken, "", map[string]string{"code": initialCode})
	if status != http.StatusConflict {
		t.Fatalf("accept with historical code: status %d body %s, want 409", status, body)
	}
	assertInvitationError(t, body, "invitation_code_invalidated")
	assertContractResponse(t, http.MethodPost, acceptPath, status, body)
...
func TestAcceptInvitationRejectsForwardedExpiredRevokedAndExhaustedCodes(t *testing.T) {
	...
	for attempt := 1; attempt <= attemptLimit; attempt++ {
		...
		status, body, headers = invitationCommandFromIP(t, handler, http.MethodPost, attemptPath, inviteeToken, "", map[string]string{"code": wrongCode})
	...
	status, body, headers := invitationCommandFromIP(t, handler, http.MethodPost, attemptPath, inviteeToken, "", map[string]string{"code": attemptCode})
	if status != http.StatusConflict {
