...
// SessionService handles the session aggregate's lifecycle commands; the
// editable draft is device-local state (ADR-0017), so the session surface has
// no stored draft to read or write.
...
// Create opens one empty private session. NormalizeSessionName keeps
