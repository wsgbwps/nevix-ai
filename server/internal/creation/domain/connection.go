...
// Provider connection state vocabulary (spec #150 / ADR-0016): AdminState is the
// governance switch, CredentialState the instance-level key, each media capability
// degrades independently, and "needs attention" is a derived hint, never persisted
// state.
type (
...
// ProviderCredentialEnvelope is the versioned AEAD envelope stored beside the connection: master key
// ID, per-seal random nonce, ciphertext. AAD binding (connection id, provider, purpose) is enforced
...
// ProviderConnection is the instance's single AI Provider Connection aggregate root. One active
// connection serves both image and video with no fallback; the plaintext Provider Key exists only
// inside Go during a check or a provider call and never leaves the encrypted envelope.
...
// CredentialVault is the port owning the master key lifecycle and the AEAD envelope codec (ADR-0016
// 本地 AEAD). EnsureKey runs only on the explicit reauthenticated paths where a new ciphertext
// follows; LoadKey never writes, so a lost key under existing ciphertext is an error the service
// fails closed on — never silent regeneration.
...
// MediaCapabilityView is one media's member-facing projection: status, a stable machine reason while
// unavailable, and one stable action advice. Endpoint, model ids, credential detail, and provider
// diagnostics never appear here (spec #150 Member surface).
...
// DeriveMediaCapabilities projects the active connection into the member surface. The evaluation order
// is fixed so one snapshot yields one stable cause per media: checking, then credential verdicts, then
// pause, then each media's own visibility.
