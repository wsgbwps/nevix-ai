...
func (h *harness) resetObjectStorageConnections(t *testing.T) {
	...
	h.storageMu.Unlock()
	if _, err := h.ownerPool.Exec(h.ctx, `TRUNCATE public.creation_generation_tasks, public.creation_reference_material_uploads, public.object_storage_connections CASCADE`); err != nil {
