...
## 更新 — 2026-09-18：统一 PR 与风险分级授权

本次更新取代本文 2026-04-30、2026-08-25 与 2026-08-28 的所有
direct-main 快道决定。所有 tracked 改动统一在短命任务分支完成并通过 PR
进入 `main`；本地 hooks 无条件拦截在 `main` 的提交与直推，不再维护路径
白名单。

Agent 默认可执行可逆的调查、实现、测试、本地提交、分支推送、PR 创建与
更新、评审处理、CI 等待，以及低/中风险合并；用户的明确指令可收窄该授权。
高风险仅包括破坏性或不可逆持久数据操作、生产部署/发布、密钥或权限/授权/
安全边界变更、付费或周期性外部资源，以及破坏性公共契约变更。Agent 仍完成
其调查、实现、不改变高风险外部或系统状态的测试、评审和 PR 准备；执行任何
高风险外部或系统动作前请求批准，包括合并高风险 PR。

本更新同时取代本文关于 `main` push 重跑与 tree-SHA 去重的决定。路径感知
`CI gate` 仅由面向 `main` 的 pull request 触发；PR 通过后合并，不再为同一
变更运行合并后检查或维护去重逻辑。
