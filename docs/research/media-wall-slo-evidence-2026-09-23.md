...
需要说清楚的是：**本次运行的产物本身无法证明这个排除生效**——8 张卡片在 1280×768 里
正好是一行，所以 60 次测量全部报 8/8，没有任何一张被排除。排除行为是在真实 Chromium 里
单独验证的：把 HEAD 上这个 observer 的源码抽出来，喂给一面 12 张卡片的溢出墙面，它只
标记了 2 张（index 0、1）。另有一个良性偏差：卡片只要在该阶段内**曾经**相交就会被计入，
所以被后续布局位移"顶上来"的屏外卡片会被算进去——这个方向只会造成误判失败，不会造成
误判通过。
...
本文件引用的四个数字来自其中一次运行。一次独立的 QA 复跑在 HEAD 上得到
46/48、77/80、87/93、1/2 ms——**结论一致，个位数毫秒的差异是运行间波动**，不是版本差异。
引用具体某一次而不是区间，是因为这样每个数字都能追到一份产物。
