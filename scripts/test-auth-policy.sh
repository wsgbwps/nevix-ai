...
hibp_fixture_container="nevix_auth_policy_hibp_fixture"
hibp_fixture_image="node:22.22.2-alpine3.23@sha256:8ea2348b068a9544dae7317b4f3aafcdc032df1647bb7d768a05a5cad1a7683f"
hibp_fixture_directory="$policy_project_root/hibp-fixture"
hibp_fixture_owned=0
...
stop_hibp_fixture() {
  if [[ "$hibp_fixture_owned" == "1" ]]; then
    docker rm --force "$hibp_fixture_container" >/dev/null
    hibp_fixture_owned=0
  fi
}
...
create_hibp_fixture_certificate() {
  mkdir -p "$hibp_fixture_directory"
  local openssl_config="$hibp_fixture_directory/openssl.cnf"
  ...
  printf '%s\n' \
    ...
    'prompt = no' \
    '[subject]' \
    'commonName = api.pwnedpasswords.com' \
    '[server_certificate]' \
    'subjectAltName = DNS:api.pwnedpasswords.com' \
  ...
  openssl req -x509 -newkey rsa:2048 -nodes -days 1 \
    ...
    -keyout "$hibp_fixture_directory/server-key.pem" \
    -out "$hibp_fixture_directory/server-cert.pem" \
    >/dev/null 2>&1
  chmod 0644 \
    "$hibp_fixture_directory/server-key.pem" \
    "$hibp_fixture_directory/server-cert.pem"
}

wait_for_hibp_fixture() {
  local mode="$1"
  local marker="ready - controlled HIBP ${mode} fixture"

  for _ in $(seq 1 120); do
    if docker logs "$hibp_fixture_container" 2>&1 | contains_fixed_string "$marker"; then
      return
    fi
    if ! docker container inspect "$hibp_fixture_container" >/dev/null 2>&1; then
      echo "error: the controlled HIBP fixture stopped before becoming ready" >&2
      return 1
    fi
    sleep 0.25
  done

  echo "error: the controlled HIBP fixture did not become ready" >&2
  docker logs "$hibp_fixture_container" >&2 || true
  return 1
}
...
start_hibp_fixture() {
  ...
  docker run --detach \
    --name "$hibp_fixture_container" \
    --network "supabase_network_${policy_project_id}" \
    --network-alias api.pwnedpasswords.com \
    --mount "type=bind,src=$repo_root/scripts/auth-policy-harness.mjs,dst=/workspace/auth-policy-harness.mjs,readonly" \
    --mount "type=bind,src=$hibp_fixture_directory,dst=/fixture,readonly" \
    --env NEVIX_AUTH_POLICY_HIBP_FIXTURE_MODE="$mode" \
    --env NEVIX_AUTH_POLICY_HIBP_CERT=/fixture/server-cert.pem \
    --env NEVIX_AUTH_POLICY_HIBP_KEY=/fixture/server-key.pem \
    --entrypoint node \
    "$hibp_fixture_image" \
    /workspace/auth-policy-harness.mjs hibp-fixture \
    >/dev/null
  hibp_fixture_owned=1
  wait_for_hibp_fixture "$mode"
}
...
start_auth_container() {
  ...
  case "$mode" in
    ...
    normal | fail-open)
      mode_args+=(
        --mount "type=bind,src=$hibp_fixture_directory/server-cert.pem,dst=/fixture/hibp-ca.pem,readonly"
        --env SSL_CERT_FILE=/fixture/hibp-ca.pem
      )
      ;;
    *)
...
node --test "$repo_root/scripts/tests/auth-policy-harness.test.mjs"
node "$repo_root/scripts/auth-policy-harness.mjs" parity
...
if docker container inspect "$hibp_fixture_container" >/dev/null 2>&1; then
  echo "error: controlled HIBP fixture '$hibp_fixture_container' already exists" >&2
  exit 1
fi
